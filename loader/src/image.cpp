...
ImageParams::ImageParams(int channelCount, int height, int width,
...
void ImageParams::getDistortionValues(cv::RNG &rng, const Size2i &inputSize, AugParams *agp) {
...
const Size2i ImageParams::getSize() {
    return Size2i(_width, _height);
}

ImageIngestParams::ImageIngestParams(bool resizeAtIngest, bool lossyEncoding,
...
Image::Image(ImageParams *params, ImageIngestParams* ingestParams, int id)
...
void Image::transform(char* item, int itemSize, char* buf, int bufSize) {
    Mat decodedImage;
...
void Image::ingest(char** dataBuf, int* dataBufLen, int* dataLen) {
    if (_ingestParams == 0) {
        return;
    }
...
void Image::save_binary(char *filn, char* item, int itemSize, char* buf) {
...
void Image::decode(char* item, int itemSize, Mat* dst) {
...
void Image::transformDecodedImage(const Mat& decodedImage, char* buf, int bufSize){
    Mat rotatedImage;
...
void Image::rotate(const Mat& input, Mat& output, int angle) {
    if (angle == 0) {
        output = input;
    } else {
...
void Image::resize(const Mat& input, Mat& output, const Size2i& size) {
...
*/
void Image::lighting(Mat& inout, float pixelstd[]) {
...
void Image::cbsjitter(Mat& inout, float cbs[]) {
    // Skip transformations if given deterministic settings
...
void Image::split(Mat& img, char* buf, int bufSize) {
...
void Image::createRandomAugParams(const Size2i& size) {

...
TEST(loader,decode) {
    shared_ptr<ImageParams> imgp = make_shared<ImageParams>(3, 128, 128, false, true, // channels, h, w, augment, flip
        20, 100,   // Scale Params
        60, 140,    // Contrast params
        -10, 10,       // Rotation params
        133,          // Aspect Ratio
        false, 0, 0, 0, 0);  // subtract mean, r, g, b, gray means
    shared_ptr<ImageIngestParams> iip = make_shared<ImageIngestParams>(true, true, 256, 256);
    ...
    int label_idx = *reinterpret_cast<int *>(labels->data());
    // We'll do 10 decodings of the same image;
    Image decoder(imgp.get(), iip.get(), 0);
    int num_decode = 10;
    ...
    for (int i = 0; i < num_decode; i++) {
        decoder.transform(data->data(), data->size(), &outbuf[i * num_pixels], num_pixels);
    }

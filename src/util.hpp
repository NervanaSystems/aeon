...
#define DUMP_VALUE(a) cout << __FILE__ << " " << __LINE__ << " " #a " " << a << endl;
    ...
    std::vector<unsigned> parse_cpu_list(const std::string& cpu_list, unsigned hardware_concurrency);
    std::vector<unsigned> get_thread_affinity_map(const std::string& config_cpu_list,
